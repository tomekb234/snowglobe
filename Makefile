...
build/main.o: src/main.cpp include/input.hpp include/diagnostic.hpp include/ast.hpp gen/parser.cpp | build
	$(CXXC) $< -o $@

build/input.o: src/input.cpp include/input.hpp | build
	$(CXXC) $< -o $@

build/lexer.o: gen/lexer.cpp include/input.hpp include/diagnostic.hpp include/ast.hpp gen/parser.cpp | build
	$(CXXC) $< -o $@

build/parser.o: gen/parser.cpp include/input.hpp include/diagnostic.hpp include/ast.hpp | build
	$(CXXC) $< -o $@
